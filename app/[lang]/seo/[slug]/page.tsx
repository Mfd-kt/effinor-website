...
import { isValidLang, getDictionary, Lang } from '@/lib/i18n';
...
export async function generateMetadata({ params }: SeoPageProps): Promise<Metadata> {
  ...
  const content = await getSeoContentBySlug(slug, lang as Lang);
...
export default async function SeoPage({ params }: SeoPageProps) {
  ...
  const content = await getSeoContentBySlug(slug, validLang);
